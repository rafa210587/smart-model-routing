...
import { type ExecutionOrigin, type RoutingDecision, type RoutingScope } from "./models.js";
...
export class SmartRouter {
  ...
  async route(prompt: string, origin: ExecutionOrigin = "main", scope: RoutingScope = "main"): Promise<RoutingDecision> {
    try {
      ...
      const candidate = this.registry?.resolve(intent.tier, intent.requiredCapabilities, scope);
      const model = candidate?.logicalModel ?? (intent.tier === "low" ? "haiku" : intent.tier === "standard" ? "sonnet" : "opus");
      return { model, tier: intent.tier, reason: intent.reason, confidence: intent.confidence, source: intent.source, subagentType: candidate?.subagentType };
    } catch {
